...
public class MainFrame extends javax.swing.JFrame {
    ...
    private void btnCrearArchivoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCrearArchivoActionPerformed
	    ...
	    if (nombre != null && tamStr != null) {
		    ...
		    int destino = 0;
		    for (int i = 0; i < 100; i++) {
			    if (!disco.getBloques()[i].isOcupado()) {
				    destino = i;
				    break;
			    }
		    }
    ...
    private void btnCargarJSONActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCargarJSONActionPerformed
	    ...
	    selector.setFileFilter(new javax.swing.filechooser.FileNameExtensionFilter("Archivos JSON", "json"));
	    ...
	    if (selector.showOpenDialog(this) == javax.swing.JFileChooser.APPROVE_OPTION) {
		    ...
		    while (!colaBloqueados.estaVacia()) {
			    colaBloqueados.desencolar();
		    }
		    while (!colaES.estaVacia()) {
			    colaES.desencolar();
		    }

		    SimuladorLoader.cargarEscenarioJSON(selector.getSelectedFile(), disco, arbolLogic, colaListos, scheduler);

		    actualizarPantalla();
		    actualizarJTree();
		    txtJournal.append(">>> SISTEMA: JSON de prueba cargado correctamente.\n");
	    }
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
		...
		if (!rbAdmin.isSelected()) {
			JOptionPane.showMessageDialog(this, "Solo los administradores pueden actualizar archivos.", "Acceso Denegado", JOptionPane.ERROR_MESSAGE);
			return;
		}
		String nombreActual = JOptionPane.showInputDialog(this, "Nombre del archivo a actualizar:");
		if (nombreActual == null || nombreActual.trim().isEmpty()) {
			return;
		}
		NodeFS nodo = buscarNodoEnArbol(arbolLogic.getRaiz(), nombreActual);
		if (nodo == null || nodo.isEsDirectorio()) {
			JOptionPane.showMessageDialog(this, "Archivo no encontrado o es una carpeta.", "Error", JOptionPane.WARNING_MESSAGE);
			return;
		}
		String nuevoNombre = JOptionPane.showInputDialog(this, "Ingrese el NUEVO nombre para el archivo:");
		if (nuevoNombre == null || nuevoNombre.trim().isEmpty()) {
			return;
		}
		txtJournal.append("[PENDIENTE] ACTUALIZAR ARCHIVO: " + nombreActual + " -> " + nuevoNombre + "\n");
		PCB solicitud = new PCB("ACTUALIZAR", nodo, nodo.getBloqueInicial(), nuevoNombre, 0);
		colaListos.encolar(solicitud);
...
// Método auxiliar para navegar el árbol hijo-hermano y llenar la tabla
	...
	private NodeFS buscarNodoEnArbol(NodeFS actual, String nombreBuscado) {
		if (actual == null) {
			return null;
		}

		// Si encontramos el archivo/directorio, lo retornamos
		if (actual.getNombre().equals(nombreBuscado)) {
			return actual;
		}

		// Si no, buscamos recursivamente en sus hijos y hermanos
		NodeFS hijo = actual.getPrimerHijo();
		while (hijo != null) {
			NodeFS encontrado = buscarNodoEnArbol(hijo, nombreBuscado);
			if (encontrado != null) {
				return encontrado;
			}
			hijo = hijo.getSiguienteHermano();
